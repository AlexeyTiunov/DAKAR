		...
		foreach ($this->result as $key=>$product)
		{
		   ...
		   //if (isset($product['Quantity']) && intval($product['Quantity']) == 0) continue; 
            ...
            //if ((isset($product['Quantity']) && intval($product['Quantity']) == 0)||(isset($product["SupplierCode"]) && iconv  ('utf8','cp1251',$product["SupplierCode"])!="KLOD" && iconv('utf8','cp1251',$product["SupplierCode"])!="USAF") )
                 //continue;
if ((isset($product['Quantity']) && intval($product['Quantity']) == 1000)||(isset($product["SupplierCode"]) && $product["SupplierCode"]!="KLOD" && $product["SupplierCode"]!="USAF" && $product["SupplierCode"]!="STOK" && $product["SupplierCode"]!="USAM" ) ) continue;     //TechnoMir
		...
		#echo "</pre>";
            ...
            $product['REGION'] = self::$log[$this->params['client']]['region'];     
                ...
                case 'tehnomir':
					...
					$product['ICODE'] = $product['Number'];
					$product['DELIVERY'] = ($product['SupplierCode']=='USAF' or $product['SupplierCode']=='USAM' )? '14':$product['DeliveryTime'] + 1;
            ...
            $product['PRICEREGIONP'] = ($product['SupplierCode']=='USAF' or $product['SupplierCode']=='USAM') ?$product['Price']-($product['Price']*0.03): /*$sqlRes['koefPrice']**/$product['Price'];
