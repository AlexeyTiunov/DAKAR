<?
 ...
 function  RecieveModelsTypesArrayFromTecDoc($tecdoc_brand_id,$tecdoc_model_id)
 {
   ...
   $ConditionForTYP_MOD_ID =MakeConditionForTYP_MOD_ID($tecdoc_model_id); 
     $sql="SELECT    TYP_ID,tof_des_texts2.TEX_TEXT as DRIVE_TYPE ,tof_des_texts.TEX_TEXT  AS CAPTION, TYP_PCON_START ,TYP_PCON_END
           FROM               tof_types
    ...
    while ($modelTypeArray=$result->Fetch())
    {
      ...
      $findedModelTypesArray[$modelTypeArray['TYP_ID']]['TRANSMISSION']=$modelTypeArray['DRIVE_TYPE']; 
 ...
 function  ShowModelsTypesArrayFromTecDoc($findedModelsTypesArray,$table_name_car_models_types)
 {
    ...
    $modelsTypesString='';    
     foreach ($findedModelsTypesArray as $tecdoc_model_type_id=>$item)
     {
         ...
         {
             $modelsTypesString.= "<input type='checkbox'  checked='checked' class='tecdoc_finded_models_types' id='{$item['ID']}' ><a title=''>{$item['CAPTION']}-{$item['DATE_BEGIN']}-{$item['DATE_END']} </a> </input><br>";
              ...
              <div id='group_type_div' style="width:30%;height:50%;float:left; margin-left:5px;"> 
               ...
               <div id='group_type_div' style="width:30%;height:50%;float:left; margin-left:5px;">
                  ...
                  <div name='tecdoc_model_id_div'>
                          <?
                          echo  ShowOptionModelsArray(RecieveModelsByBrand(20827,$carmodel) )
